...
+                <div className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-amber-500/20 text-amber-300 border border-amber-500/30 ml-2">
+                  Personnalisé
+                </div>
+              )}
             </div>
...
-            </button>
+            {isSpinning ? (
+              <LoadingSpinner size="md" text="Sélection en cours..." />
+            ) : (
+              <button
+                onClick={onSpin}
+                className="bg-gradient-to-r from-amber-500 to-orange-500 active:from-amber-600 active:to-orange-600 text-white font-bold py-4 px-6 sm:px-8 rounded-xl transition-all duration-200 shadow-lg flex items-center gap-3 mx-auto mobile-button touch-action-none"
+              >
+                <Play className="w-6 h-6" />
+                <span className="text-sm sm:text-base">Tourner la Roue</span>
+              </button>
+            )}
           </div>
         </div>
